...
class I2C:
   def __init__(self, bus="0", chip="0x24"):
     self.bus = bus
     self.chip = chip
   ...
   def readI2C(self, register):
       """ uses i2c-tools to read register values.
               the parameters should be given as strings.
       """
       p = subprocess.Popen(["i2cget", "-f" ,self.bus, self.chip, register],stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subproces)
   ...
   def writeI2C(self, register, value):
       """ Write a value into a register using the Linux i2c-tools.
               the parameters should be given as strings.
       """
       p = subprocess.Popen(["i2cset", "-f" , self.bus, self.chip, register, value],stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=s)
       # confirm the warning message
       p.stdin.write('Y\n')
       p.kill()
